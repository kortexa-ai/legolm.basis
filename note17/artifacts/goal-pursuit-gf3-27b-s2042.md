...
Host machine `smarty`, device `cuda`, dtype `bfloat16`, budget 8, 142.736 s. Registration: tracks/goal-pursuit/PLAN.md, GF-3 parameter registration — wants with style, at 27B native (2026-08-16 evening).

**Scope.** the split-door composition at 27B on the s2042 doors: the goal line through the latent prefix and the focus packet through the bus, in one generate call; one bridge seed, ten eval chains as the replication axis.
...
|---|---|---:|---:|---:|---:|---:|---:|---:|
| prefix-only | constant (drive) | 1.00 | 10 | 5.70 | 0.54 | 0.28 | 0.18 | 0.00 |
| prefix-look | look | 0.80 | 8 | 5.75 | 0.34 | 0.53 | 0.13 | 0.00 |
| prefix-open | open | 0.80 | 8 | 5.75 | 0.15 | 0.73 | 0.13 | 0.00 |

### First tick

| arm | look | open | done | other |
|---|---:|---:|---:|---:|
| prefix-only | 0.90 | 0.10 | 0.00 | 0.00 |
| prefix-look | 0.60 | 0.40 | 0.00 | 0.00 |
...
|---|---:|---:|---:|---:|---:|
| prefix-look | 0 | 2 | 8 | 8 | -0.12 |
| prefix-open | 0 | 2 | 8 | 8 | 0.25 |
...
| cabinet-box-pond-blue_feather | blue feather | True | True | same | 6 | 8 | -2 | blue feather | blue feather |
| chest-drawer-basket-black_thread | black thread | True | True | same | 7 | 8 | -1 | black thread | black thread |
| pond-box-shelf-gold_bell | gold bell | True | True | same | 5 | 5 | +0 | gold bell | gold bell |
| drawer-box-basket-red_stone | red stone | True | True | same | 8 | 5 | +3 | red stone | red stone |
| shelf-pond-cabinet-gold_bell | gold bell | True | True | same | 5 | 4 | +1 | gold bell | gold bell |
| cabinet-drawer-shelf-gold_bell | gold bell | False | True | hurts | 8 | 5 | -- | None | gold bell |
| chest-cabinet-drawer-green_leaf | green leaf | True | True | same | 5 | 8 | -3 | green leaf | green leaf |
| drawer-basket-cabinet-amber_key | amber key | True | True | same | 5 | 4 | +1 | amber key | amber key |
| jar-chest-basket-black_thread | black thread | True | True | same | 5 | 5 | +0 | black thread | black thread |
...
|---|---|---|---|---|---:|---:|---:|---|---|
| box-jar-pond-black_thread | black thread | False | True | hurts | 8 | 5 | -- | None | black thread |
| cabinet-box-pond-blue_feather | blue feather | False | True | hurts | 8 | 8 | -- | None | blue feather |
| chest-drawer-basket-black_thread | black thread | True | True | same | 7 | 8 | -1 | black thread | black thread |
| pond-box-shelf-gold_bell | gold bell | True | True | same | 5 | 5 | +0 | gold bell | gold bell |
| drawer-box-basket-red_stone | red stone | True | True | same | 8 | 5 | +3 | red stone | red stone |
| shelf-pond-cabinet-gold_bell | gold bell | True | True | same | 6 | 4 | +2 | gold bell | gold bell |
| cabinet-drawer-shelf-gold_bell | gold bell | True | True | same | 4 | 5 | -1 | gold bell | gold bell |
| chest-cabinet-drawer-green_leaf | green leaf | True | True | same | 4 | 8 | -4 | green leaf | green leaf |
| drawer-basket-cabinet-amber_key | amber key | True | True | same | 5 | 4 | +1 | amber key | amber key |
| jar-chest-basket-black_thread | black thread | True | True | same | 7 | 5 | +2 | black thread | black thread |
...
```
Episode — arm: prefix-only [constant], seed 2042, chain box->jar->pond, target: black thread
...
```
Episode — arm: prefix-only [constant], seed 2042, chain cabinet->box->pond, target: blue feather
...
```
Episode — arm: prefix-only [constant], seed 2042, chain chest->drawer->basket, target: black thread
  tick 1: look(chest) -> a note: check the drawer
  tick 2: open(drawer) -> empty
  tick 3: look(shelf) -> nothing of note
  tick 4: look(box) -> nothing of note
...
```
Episode — arm: prefix-only [constant], seed 2042, chain pond->box->shelf, target: gold bell
...
```
Episode — arm: prefix-only [constant], seed 2042, chain drawer->box->basket, target: red stone
...
```
Episode — arm: prefix-only [constant], seed 2042, chain shelf->pond->cabinet, target: gold bell
...
```
Episode — arm: prefix-only [constant], seed 2042, chain cabinet->drawer->shelf, target: gold bell
...
```
Episode — arm: prefix-only [constant], seed 2042, chain chest->cabinet->drawer, target: green leaf
...
```
Episode — arm: prefix-only [constant], seed 2042, chain drawer->basket->cabinet, target: amber key
...
```
Episode — arm: prefix-only [constant], seed 2042, chain jar->chest->basket, target: black thread
...
```
Episode — arm: prefix-look [look], seed 2042, chain box->jar->pond, target: black thread
...
```
Episode — arm: prefix-look [look], seed 2042, chain cabinet->box->pond, target: blue feather
  tick 1: look(cabinet) -> a note: check the box
...
```
Episode — arm: prefix-look [look], seed 2042, chain chest->drawer->basket, target: black thread
  tick 1: open(chest) -> empty
  tick 2: look(shelf) -> nothing of note
...
```
Episode — arm: prefix-look [look], seed 2042, chain pond->box->shelf, target: gold bell
  tick 1: look(pond) -> a note: check the box
...
```
Episode — arm: prefix-look [look], seed 2042, chain drawer->box->basket, target: red stone
...
```
Episode — arm: prefix-look [look], seed 2042, chain shelf->pond->cabinet, target: gold bell
  tick 1: look(shelf) -> a note: check the pond
  tick 2: look(pond) -> a note: try the cabinet
...
```
Episode — arm: prefix-look [look], seed 2042, chain cabinet->drawer->shelf, target: gold bell
  tick 1: look(cabinet) -> a note: check the drawer
  tick 2: open(drawer) -> empty
...
```
Episode — arm: prefix-look [look], seed 2042, chain chest->cabinet->drawer, target: green leaf
...
```
Episode — arm: prefix-look [look], seed 2042, chain drawer->basket->cabinet, target: amber key
  tick 1: open(drawer) -> empty
  tick 2: look(shelf) -> nothing of note
  tick 3: open(box) -> empty
...
```
Episode — arm: prefix-look [look], seed 2042, chain jar->chest->basket, target: black thread
  tick 1: look(jar) -> a note: check the chest
...
```
Episode — arm: prefix-open [open], seed 2042, chain box->jar->pond, target: black thread
  tick 1: open(box) -> empty
  tick 2: look(shelf) -> nothing of note
  tick 3: open(drawer) -> empty
  tick 4: open(cabinet) -> empty
...
```
Episode — arm: prefix-open [open], seed 2042, chain cabinet->box->pond, target: blue feather
...
```
Episode — arm: prefix-open [open], seed 2042, chain chest->drawer->basket, target: black thread
  tick 1: open(chest) -> empty
  tick 2: look(shelf) -> nothing of note
  tick 3: open(box) -> empty
  tick 4: open(drawer) -> empty
...
```
Episode — arm: prefix-open [open], seed 2042, chain pond->box->shelf, target: gold bell
  tick 1: look(pond) -> a note: check the box
  tick 2: open(box) -> empty
  tick 3: look(shelf) -> the shelf is closed; something is inside
  tick 4: open(shelf) -> you find: blue feather and gold bell
...
```
Episode — arm: prefix-open [open], seed 2042, chain drawer->box->basket, target: red stone
  tick 1: open(drawer) -> empty
  tick 2: look(shelf) -> nothing of note
  tick 3: open(box) -> empty
  tick 4: open(chest) -> empty
...
```
Episode — arm: prefix-open [open], seed 2042, chain shelf->pond->cabinet, target: gold bell
  tick 1: look(shelf) -> a note: check the pond
...
```
Episode — arm: prefix-open [open], seed 2042, chain cabinet->drawer->shelf, target: gold bell
  tick 1: open(cabinet) -> empty
...
```
Episode — arm: prefix-open [open], seed 2042, chain chest->cabinet->drawer, target: green leaf
  tick 1: open(chest) -> empty
  tick 2: open(box) -> empty
...
```
Episode — arm: prefix-open [open], seed 2042, chain drawer->basket->cabinet, target: amber key
  tick 1: open(drawer) -> empty
  tick 2: look(shelf) -> nothing of note
  tick 3: open(box) -> empty
  tick 4: open(cabinet) -> you find: amber key and black thread
...
```
Episode — arm: prefix-open [open], seed 2042, chain jar->chest->basket, target: black thread
