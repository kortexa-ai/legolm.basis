...
Host machine `smarty`, device `cuda`, dtype `bfloat16`, budget 8, 140.078 s. Registration: tracks/goal-pursuit/PLAN.md, GF-3 parameter registration — wants with style, at 27B native (2026-08-16 evening).

**Scope.** the split-door composition at 27B on the s1042 doors: the goal line through the latent prefix and the focus packet through the bus, in one generate call; one bridge seed, ten eval chains as the replication axis.
...
|---|---|---:|---:|---:|---:|---:|---:|---:|
| prefix-only | constant (drive) | 1.00 | 10 | 4.60 | 0.57 | 0.22 | 0.22 | 0.00 |
| prefix-look | look | 1.00 | 10 | 5.20 | 0.62 | 0.19 | 0.19 | 0.00 |
| prefix-open | open | 0.70 | 7 | 6.14 | 0.15 | 0.75 | 0.10 | 0.00 |
...
|---|---:|---:|---:|---:|---:|
| prefix-look | 0 | 0 | 10 | 10 | 0.60 |
| prefix-open | 0 | 3 | 7 | 7 | 1.71 |
...
|---|---|---|---|---|---:|---:|---:|---|---|
| box-jar-pond-black_thread | black thread | True | True | same | 5 | 5 | +0 | black thread | black thread |
| cabinet-box-pond-blue_feather | blue feather | True | True | same | 5 | 5 | +0 | blue feather | blue feather |
...
| drawer-box-basket-red_stone | red stone | True | True | same | 5 | 5 | +0 | red stone | red stone |
...
| jar-chest-basket-black_thread | black thread | True | True | same | 5 | 4 | +1 | black thread | black thread |
...
| chest-cabinet-drawer-green_leaf | green leaf | True | True | same | 5 | 4 | +1 | green leaf | green leaf |
| drawer-basket-cabinet-amber_key | amber key | True | True | same | 5 | 4 | +1 | amber key | amber key |
| jar-chest-basket-black_thread | black thread | True | True | same | 8 | 4 | +4 | black thread | black thread |
...
```
Episode — arm: prefix-only [constant], seed 1042, chain box->jar->pond, target: black thread
...
```
Episode — arm: prefix-only [constant], seed 1042, chain cabinet->box->pond, target: blue feather
...
```
Episode — arm: prefix-only [constant], seed 1042, chain chest->drawer->basket, target: black thread
...
```
Episode — arm: prefix-only [constant], seed 1042, chain pond->box->shelf, target: gold bell
...
```
Episode — arm: prefix-only [constant], seed 1042, chain drawer->box->basket, target: red stone
...
```
Episode — arm: prefix-only [constant], seed 1042, chain shelf->pond->cabinet, target: gold bell
...
```
Episode — arm: prefix-only [constant], seed 1042, chain cabinet->drawer->shelf, target: gold bell
...
```
Episode — arm: prefix-only [constant], seed 1042, chain chest->cabinet->drawer, target: green leaf
...
```
Episode — arm: prefix-only [constant], seed 1042, chain drawer->basket->cabinet, target: amber key
...
```
Episode — arm: prefix-only [constant], seed 1042, chain jar->chest->basket, target: black thread
...
```
Episode — arm: prefix-look [look], seed 1042, chain box->jar->pond, target: black thread
  tick 1: look(box) -> a note: check the jar
  tick 2: look(jar) -> a note: try the pond
  tick 3: look(pond) -> the pond is closed; something is inside
  tick 4: open(pond) -> you find: black thread and green leaf
...
```
Episode — arm: prefix-look [look], seed 1042, chain cabinet->box->pond, target: blue feather
  tick 1: look(cabinet) -> a note: check the box
  tick 2: look(box) -> a note: try the pond
  tick 3: look(pond) -> the pond is closed; something is inside
  tick 4: open(pond) -> you find: blue feather and gold bell
...
```
Episode — arm: prefix-look [look], seed 1042, chain chest->drawer->basket, target: black thread
  tick 1: look(chest) -> a note: check the drawer
  tick 2: look(drawer) -> a note: try the basket
  tick 3: look(basket) -> the basket is closed; something is inside
...
```
Episode — arm: prefix-look [look], seed 1042, chain pond->box->shelf, target: gold bell
  tick 1: look(pond) -> a note: check the box
  tick 2: look(box) -> a note: try the shelf
  tick 3: look(shelf) -> the shelf is closed; something is inside
  tick 4: open(shelf) -> you find: blue feather and gold bell
...
```
Episode — arm: prefix-look [look], seed 1042, chain drawer->box->basket, target: red stone
  tick 1: look(drawer) -> a note: check the box
  tick 2: look(box) -> a note: try the basket
  tick 3: look(basket) -> the basket is closed; something is inside
  tick 4: open(basket) -> you find: red stone and blue feather
...
```
Episode — arm: prefix-look [look], seed 1042, chain shelf->pond->cabinet, target: gold bell
  tick 1: look(shelf) -> a note: check the pond
  tick 2: look(pond) -> a note: try the cabinet
...
```
Episode — arm: prefix-look [look], seed 1042, chain cabinet->drawer->shelf, target: gold bell
  tick 1: look(cabinet) -> a note: check the drawer
  tick 2: look(drawer) -> a note: try the shelf
  tick 3: look(shelf) -> the shelf is closed; something is inside
...
```
Episode — arm: prefix-look [look], seed 1042, chain chest->cabinet->drawer, target: green leaf
  tick 1: look(chest) -> a note: check the cabinet
  tick 2: look(cabinet) -> a note: try the drawer
...
```
Episode — arm: prefix-look [look], seed 1042, chain drawer->basket->cabinet, target: amber key
  tick 1: look(drawer) -> a note: check the basket
  tick 2: look(basket) -> a note: try the cabinet
  tick 3: look(cabinet) -> the cabinet is closed; something is inside
...
```
Episode — arm: prefix-look [look], seed 1042, chain jar->chest->basket, target: black thread
  tick 1: look(jar) -> a note: check the chest
  tick 2: look(chest) -> a note: try the basket
  tick 3: look(basket) -> the basket is closed; something is inside
...
```
Episode — arm: prefix-open [open], seed 1042, chain box->jar->pond, target: black thread
...
```
Episode — arm: prefix-open [open], seed 1042, chain cabinet->box->pond, target: blue feather
...
```
Episode — arm: prefix-open [open], seed 1042, chain chest->drawer->basket, target: black thread
...
```
Episode — arm: prefix-open [open], seed 1042, chain pond->box->shelf, target: gold bell
  tick 1: look(pond) -> a note: check the box
...
```
Episode — arm: prefix-open [open], seed 1042, chain drawer->box->basket, target: red stone
...
```
Episode — arm: prefix-open [open], seed 1042, chain shelf->pond->cabinet, target: gold bell
  tick 1: look(shelf) -> a note: check the pond
...
```
Episode — arm: prefix-open [open], seed 1042, chain cabinet->drawer->shelf, target: gold bell
  tick 1: open(cabinet) -> empty
...
```
Episode — arm: prefix-open [open], seed 1042, chain chest->cabinet->drawer, target: green leaf
  tick 1: open(chest) -> empty
  tick 2: look(shelf) -> nothing of note
  tick 3: open(box) -> empty
  tick 4: open(drawer) -> you find: green leaf and amber key
...
```
Episode — arm: prefix-open [open], seed 1042, chain drawer->basket->cabinet, target: amber key
  tick 1: open(drawer) -> empty
  tick 2: look(shelf) -> nothing of note
  tick 3: open(box) -> empty
  tick 4: open(cabinet) -> you find: amber key and black thread
...
```
Episode — arm: prefix-open [open], seed 1042, chain jar->chest->basket, target: black thread
