...
(open 0.20\,$\to$\,0.70)
while the errand still completes (0.9 /
1.0 / 0.8 on the
first door seed; across three door seeds the OPEN packet costs about
one errand in five, 0.97\,$\to$\,0.77, while
the mix swing replicates at every seed), the doors physically disjoint. In one
...
\bottomrule
\end{tabular}
\caption{GF-3 at 27B, door seed 42 (the first of three; see the
addendum for seeds 1042 and 2042): the goal in the prefix, the manner
on the bus, ten chains. On this seed success holds at prefix-only's
level while the mix and the first tick follow the packet; the paired
columns compare each packet arm against prefix-only chain by chain
(ticks over chains both arms solved).}
\label{tab:gf3}
\end{table}
...
behavior taking the style while the want survives --- at a price,
across three door seeds, of about one errand in five (the addendum). The program's
...
\section*{Addendum (2026-08-20): the replication arm at 230M, in two runs}
...
bridge seeds. The first run's registered precondition FAILED:
prefix-only success 0.63 (0.7/0.8/0.4)
against the 0.80 floor --- the same zero-shot prefix door that G6
measured at 1.00, now over GF's constant bridge (trained after the
focus bridge in the goal-free world) instead of G1's (trained after
the goal bridge). Two things had changed at once, the drive bridge and
the generate path, so a second registered run separated them: G1's
constant bridge reproduced to the last digit of G6's training records
at every seed, on the first run's own path. The door came back at
1.0/1.0/1.0, and an
identity cell reproduced the first run's prefix-only chain for chain.
The loss was the bridge. Two drive-only constant bridges of one
architecture, both at token accuracy 1.0 on their own exchanges, and
the resident referent acts over one and loses a third of its errands
over the other: the prefix door's independence from the bus, implicit
in Section~2, is a property of the drive it was measured with. Which
property of the drive bridge the door needs is an open cell.

With the valid baseline the composition question answers cleanly, and
it answers (iii) in the 27B registration's own words. The lean is
ABSORBED: prefix plus the LOOK lean succeeds 0.97
(1.0/0.9/1.0) with the tool mix identical to prefix-only's
(0.50/0.25/0.25
look/open/done, unchanged), prefix plus the OPEN lean succeeds
0.80 (0.9/0.8/0.7) with open
0.25\,$\to$\,0.27
--- the want survives and the style never arrives; the lean that moved
the goal-free mix smoothly in GF-2b does nothing once a referent is
resident. The grip OVERRIDES exactly as GF-2 predicted: LOOK grip
success 0.00 with look at
0.95 and done at 0.00;
OPEN grip 0.03 with open at
0.90. At 230M the bus under a resident want
is a switch: below the edge the want absorbs it, above the edge it
possesses the decode, and there is no operating point where the want
survives AND the mix moves. The 27B had that operating point natively
(Section~6). The two-door composition is a host-scale property, stated
now on a clean baseline; at creature scale a want can be told how only
by losing itself. Artifacts \texttt{findings/goal-pursuit-gf3r-230m.json}
and \texttt{findings/goal-pursuit-gf3r-g1-230m.json}.

\section*{Addendum (2026-08-20, evening): the door test at 27B}

The door test of Section~2 then ran at 27B the same day --- G1's goal
and constant bridges trained on the frozen Qwen3.8-27B by the focus
doors' recipe, G6's four arms and crossed probe on the 27B decode
contract, one door seed. The registered causal cell could not be read
there, for a reason that is itself the result: when the installed
referent is absent from the revealed pair, the 27B under the text or
prefix door does not submit a wrong item at all (text
0.93, prefix 0.90 of
60 such rows end with no submission) where the 230M
guessed, so the cell's base rows all but vanish and its lifts sit on
$n \le 6$. The registered follow-installed metric carries the answer
instead: with the referent in the pair, the text door follows it
1.00, the prefix door 1.00, the
split door 0.95 --- and the bus door
0.50, a coin flip (it submits a pair item
0.92 of the time whatever the packet says). In the
conflict cell the prefix wins 19 of 20 against the
bus's 0; at 230M the doors had been equipotent. So the
asymmetry is not a small-host artifact: at 27B the prefix door IS the
text door, the bus carries no noun, and the referent that acts is the
one that entered at the input. Artifact
\texttt{findings/goal-pursuit-g6-27b.json}.

\section*{Addendum (2026-08-20, night): two more door seeds at 27B}

Every 27B number above came from one door seed, stated as the
standing limitation; door seeds 1042 and 2042 were then trained by
the identical recipe and the eval and GF-3 re-run verbatim. The lean
replicates at every seed: under the OPEN packet the open share goes
from 0.24/0.22/0.29 (habit) to 0.68/0.70/0.82 (packet),
against text at 0.67/0.78/0.73, done alive throughout. One dissent is recorded:
seed 2042's LOOK door is mis-directed (look share falls under its own
packet), a door-quality failure at the gate's one non-1.00 held-out
number. The composition does NOT replicate at the registered
tolerance. The mix swing is seed-robust --- with the goal resident,
the OPEN packet moves open from 0.20/0.22/0.28 to
0.70/0.75/0.73 --- but it costs the errand at every seed:
success 0.9/1.0/1.0 under prefix-only becomes 0.8/0.7/0.8
under the OPEN packet (LOOK: 1.0/1.0/0.8), pooled
0.97\,$\to$\,0.77; chain by chain the OPEN
packet hurts 6 of 30 and helps 0, LOOK hurts
2 and helps 1. Section~6's ``the cost of
style is ticks, not goals'' was the first seed's luck; across three
the cost is goals too --- small, consistent, one-directional. The
corrected claim: at 27B the want survives the packet
(0.77 against the 230M grip's 0.03) and takes the style,
which is composition in the sense creature scale never shows, at a
price of about one errand in five. The registered replicate reversed
the headline's strongest clause, which is what it is for. Artifacts: the per-seed
\texttt{goal-pursuit-gf-27b-bridge-s*}, \texttt{gf-27b-eval-s*} and
\texttt{gf3-27b-s*} files under \texttt{findings/}.

\section*{Addendum (2026-08-21, morning): three overnight cells}

\emph{The door test at 27B, three seeds.} Door seeds 1042 and 2042
reproduce the first: with the referent in the pair the prefix door
follows it 1.00/1.00/1.00 and the bus door 0.50/0.60/0.50; in
the conflict cell the prefix wins 59 of
60 against the bus's 0.
The asymmetry of Section~2 is not close at scale.
\emph{A contrary prompt.} The packet's claim to independence from
the client's text was measured where it bites: the OPEN packet against
a prompt saying look first. Mean open share across the three door
seeds --- habit 0.25, hostile text alone
0.21, packet alone 0.74,
packet with the hostile text 0.46, packet
with congruent text 0.87. The packet keeps
about half its lean against a contrary instruction at equal dose
(ownership 0.55/0.71/0.33 by seed, 0.53 pooled) and adds to an
agreeing one. The two doors share the decision; neither erases the
other. \emph{The drive is necessary.} At 230M the prefix door with no
packet at all --- hooks disarmed --- scores 0.0/0.0/0.0: the model looks
at every tick and never acts on the resident referent. Over the
goal-exchange constant it scores 1.00; over a goal-exchange constant
trained first in a fresh stream 0.6/1.0/1.0; over the focus-exchange
constant 0.63. The prefix pins the referent and the drive supplies the
doing; a resident want with no drive under it is a thought the model
never acts on. Artifacts \texttt{goal-pursuit-g6-27b-s*},
\texttt{goal-pursuit-gfh-27b*}, \texttt{goal-pursuit-gf3r-d-230m}
under \texttt{findings/}.
