...
\midrule
dense (trained) & 0.72, 0.80, 0.83, 0.65, 0.75 & 0.75 \\
basis-16 (trained) & 0.91, 0.75, 0.67, 0.88, 0.91 & 0.82 \\
...
\midrule
16 & 0.17 & 0.018 {\footnotesize ($n{=}3$)} \\
32 & 0.27 {\footnotesize ($n{=}3$)} & 0.028 {\footnotesize ($n{=}3$)} \\
48 & 0.47 {\footnotesize ($n{=}3$)} & 0.032 {\footnotesize ($n{=}3$)} \\
64 & 0.59 & 0.031 \\
96 & 0.62 {\footnotesize ($n{=}3$)} & 0.063 {\footnotesize ($n{=}3$)} \\
128 & 0.64 {\footnotesize ($n{=}3$)} & 0.070 {\footnotesize ($n{=}3$)} \\
256 & 0.67 & 0.188 \\
\bottomrule
\end{tabular}
\end{table}
...
\section{Conclusion}
Reparameterizing a conditional LoRA bridge's output as coefficients over a basis of LoRA directions turns its main scaling liability into a design space with two useful regimes: a trained low-dimensional regime that compresses 16$\times$ while dissolving the diversity--quality tradeoff, and a frozen seeded regime that reduces the trainable and storable footprint to a coefficient head at 0.09\% of the dense bridge, for a measured $2\times$ training cost. A span ramp whose position grows with task complexity separates the regimes and gives practitioners a selection rule; mapping it at seven widths per task showed the boundary we first reported to be a gradual rise rather than a threshold. The failures are as instructive as the successes: the basis should be stationary or absent, not fast, and composition fails in coefficient space exactly as it does in weight space. That locates the problem in the merge operation, and makes disjoint allocation over a shared frozen basis the next constructive step.
