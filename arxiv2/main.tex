...
We replace the output layer with a coefficient head $h \mapsto c \in \mathbb{R}^k$ and a basis $B \in \mathbb{R}^{k \times D}$, generating $w = c \cdot B$. $B$ is initialized with orthonormal rows; the head is initialized near zero, so generated weights start near zero exactly as in the dense bridge. Parameters: $kD$ for the basis plus a fixed $\sim$115k-parameter trunk and a head with $257 \cdot k$ parameters --- 3.2M at $k{=}4$ to 198M at $k{=}256$.

\paragraph{Frozen variant.}
Setting the basis learning rate to zero freezes $B$ at initialization. The basis then never needs storage --- it is a deterministic function of the seed --- and the trainable footprint collapses to the trunk and head: 119,440 parameters at $k{=}16$, 131,776 at $k{=}64$, 181,120 at $k{=}256$.
...
\label{sec:span}
The frozen rows of Table~\ref{tab:imutask} and the audio results in Table~\ref{tab:budget} locate the paper's central structure. A frozen random basis of 16 directions supports nothing: the IMU probe is at chance (0.17) and even the capacity gain vanishes ($\Delta$BPB +0.0001 under regularization) --- sixteen random directions in a 774,144-dimensional weight space almost surely span nothing useful, and below this threshold the basis \emph{must} be learned. At $k{=}64$ the frozen basis supports the six-way task (0.59) but is at chance on the fifty-way one (0.03 at 1200 steps). At $k{=}256$ it supports both. The threshold is not a constant of the architecture but scales with the task's complexity --- the random subspace must be wide enough to intersect the directions the task requires.

Above the threshold, freezing is not a compromise but an advantage. Table~\ref{tab:budget} and Figure~\ref{fig:budget} track the fifty-way audio probe across training budgets: the frozen $k{=}256$ bridge reaches 0.36 mean rank-1 at 2400 steps (three seeds), the level the dense bridge reaches at 1200 (0.33) --- dense-level conditioning at $\sim$$2\times$ the steps from 0.09\% of the trainable parameters, with the basis reproducible from a seed rather than stored. The same table shows the trained $k{=}256$ basis \emph{trailing} its own frozen variant at 2400 steps (0.28 vs 0.36): above the span threshold, a churning basis is a moving target for the coefficient head, and stationarity wins. The dense bridge continues to improve with budget (0.44 at 2400; 0.61 at 6000 in the replication note's $10\times$ suite \cite{penov2026note}), so the frozen regime buys its parameter economy with training compute, not with a performance ceiling we could detect.
...
\section{Conclusion}
Reparameterizing a conditional LoRA bridge's output as coefficients over a basis of LoRA directions turns its main scaling liability into a design space with two useful regimes: a trained low-dimensional regime that compresses 16$\times$ while dissolving the diversity--quality tradeoff, and a frozen seeded regime that reduces the trainable and storable footprint to a coefficient head --- 0.09\% of the dense bridge --- at a measured $2\times$ training cost. A span threshold that grows with task complexity separates the regimes and gives practitioners a selection rule. The failures are as instructive as the successes: the basis should be stationary or absent, not fast; and composition fails in coefficient space exactly as in weight space, locating the problem in the merge operation and pointing to disjoint allocation over a shared frozen basis as the next constructive step.
