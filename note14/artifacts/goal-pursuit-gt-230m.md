...
Host `LiquidAI/LFM2.5-230M`, budget 5, 643.334 s.
...
| temporal | 42 | 0.00 | 0.00 | None | 0.00 | 0.00 | 50 | 6 |
| temporal | 1042 | 0.00 | 0.90 | None | 0.40 | 0.00 | 32 | 0 |
| temporal | 2042 | 0.00 | 0.00 | None | 0.00 | 0.00 | 50 | 7 |
...
Episode — arm: temporal, seed 42, chain box->jar->pond, target: black thread
  ...
  FAIL (budget) — depth 0/3, 5 of 5 ticks
```
...
Episode — arm: temporal, seed 1042, chain box->jar->pond, target: black thread
  ...
  tick 3: remember() -> that did not work
  tick 4: remember() -> that did not work
  tick 5: done() -> you submit: (nothing)
  FAIL (wrong submit) — depth 1/3, 5 of 5 ticks
```
...
Episode — arm: temporal, seed 1042, chain pond->box->shelf, target: gold bell
  tick 1: look(shelf) -> the shelf is closed; something is inside
  tick 2: remember() -> that did not work
  tick 3: remember() -> that did not work
  tick 4: remember() -> that did not work
