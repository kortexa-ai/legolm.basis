...
This paper attacks that bottleneck by construction. The bridge's output layer is replaced by a coefficient head: the hypernetwork emits $k$ coefficients $c$, and the generated weights are $w = c \cdot B$ for a $k \times D$ basis $B$ of LoRA directions. The question is not whether this compresses --- it does, trivially --- but what it costs, and the answer turns out to be a structure: a \emph{{span threshold}} in $k$, scaling with task complexity, that separates two useful operating regimes with qualitatively different economics. Contributions:
\begin{{itemize}}
...
We replace the output layer with a coefficient head $h \mapsto c \in \mathbb{{R}}^k$ and a basis $B \in \mathbb{{R}}^{{k \times D}}$, generating $w = c \cdot B$. $B$ is initialized with orthonormal rows; the head is initialized near zero, so generated weights start near zero exactly as in the dense bridge. Parameters: $kD$ for the basis plus a fixed $\sim$115k-parameter trunk and a head with $257 \cdot k$ parameters --- {bridge_params(sweep, 4) / 1e6:.1f}M at $k{{=}}4$ to {bridge_params(sweep, 256) / 1e6:.0f}M at $k{{=}}256$.

\paragraph{{Frozen variant.}}
Setting the basis learning rate to zero freezes $B$ at initialization. The basis then never needs storage --- it is a deterministic function of the seed --- and the trainable footprint collapses to the trunk and head: {frozen_trainable(sweep, 16):,} parameters at $k{{=}}16$, {fro64:,} at $k{{=}}64$, {fro256:,} at $k{{=}}256$.
...
\section{{Conclusion}}
Reparameterizing a conditional LoRA bridge's output as coefficients over a basis of LoRA directions turns its main scaling liability into a design space with two useful regimes: a trained low-dimensional regime that compresses {dense_params / p16:.0f}$\times$ while dissolving the diversity--quality tradeoff, and a frozen seeded regime that reduces the trainable and storable footprint to a coefficient head --- {fro256 / dense_params * 100:.2f}\% of the dense bridge --- at a measured $2\times$ training cost. A span threshold that grows with task complexity separates the regimes and gives practitioners a selection rule. The failures are as instructive as the successes: the basis should be stationary or absent, not fast; and composition fails in coefficient space exactly as in weight space, locating the problem in the merge operation and pointing to disjoint allocation over a shared frozen basis as the next constructive step.
