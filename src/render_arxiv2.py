...
def imu_task_table(sweep: dict[str, Any], dense: dict[str, Any]) -> str:
    # The dense seed-42 IMU probe lives in the dense reference suite; the rest
    # of both rows come from the sweep.
...
\begin{{itemize}}
  ...
  \item \textbf{{The span ramp}} separating them, mapped at seven widths on each of two task complexities, plus three informative negatives: faster basis learning rates destabilize training, basis training at large $k$ is mildly harmful, and coefficient-space composition inherits weight-space sub-additivity exactly.
\end{{itemize}}
...
\label{{sec:span}}
Table~\ref{{tab:span}} maps frozen-basis conditioning against $k$ at both task complexities. An earlier version of this study measured two points per task and reported a \emph{{span threshold}} bracketing them. With seven widths per task the structure is visible, and it is not a threshold: conditioning rises gradually with $k$. The IMU column is strictly increasing, from chance at $k{{=}}16$ ({fmt(stask(sweep, 'task-imu-basis-16-blr0'), 2)}) through {fmt(mean_of(seed_values(sweep, 'task-imu-basis-32-blr0')), 2)} at $k{{=}}32$ and {fmt(mean_of(seed_values(sweep, 'task-imu-basis-48-blr0')), 2)} at $k{{=}}48$ to {fmt(mean_of(seed_values(sweep, 'task-imu-basis-96-blr0')), 2)} at $k{{=}}96$, after which it flattens. The audio column increases too, apart from a single-seed $k{{=}}64$ point that sits {fmt(abs(mean_of(seed_values(sweep, 'task-audio-basis-48-s1200-blr0')) - stask(sweep, 'task-audio-basis-64-s1200-blr0')), 3)} below its neighbour, well inside the seed spread at that width. Nothing in either sequence marks a boundary; a bracket drawn between any two adjacent points would have looked like a threshold.
...
\begin{{table}}[t]
\centering
\caption{{Frozen-basis task conditioning against basis width $k$ (rank-1 accuracy, true-feature condition; IMU chance 0.17 at 600 steps, audio chance 0.02 at 1200 steps). Cells give the seed mean with $n$ where more than one seed was run; $k{{=}}16$, 64, and 256 are the original single-seed points. Both columns rise with $k$, and the audio ramp arrives later than the IMU one.}}
\label{{tab:span}}
\begin{{tabular}}{{rll}}
\toprule
