...
def make_bridge(bridge_spec: str, input_dim: int, output_dim: int) -> nn.Module:
    """Build a bridge from its spec: "dense" or "basis-<k>" (e.g. "basis-64")."""
...
def _load_bridge_setup(
    ...
    bridge = make_bridge(bridge_spec, bundle.feature_dim, total_lora_dim(model)).to(device)
...
def run_task_eval(
    ...
    for cond_name, cond_mode in conditions.items():
        ...
        if cond_mode != "none":
            bridge = make_bridge(bridge_spec, task_data.feature_dim, lora_dim).to(device)
